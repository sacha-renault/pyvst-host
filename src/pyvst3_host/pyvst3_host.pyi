# type: ignore
import sys
from typing import Literal, List, Any, Optional, Tuple, Dict
import numpy as np
import enum
import numpy


class ProcessModes(Enum):
    """Enumeration of Process Modes for VST plugin processing."""
    kRealtime: int  # Realtime processing
    kPrefetch: int  # Prefetch processing
    kOffline: int   # Offline processing

class SymbolicSampleSizes(Enum):
    """Enumeration of Symbolic Sample Sizes for VST processing."""
    kSample32: int  # 32-bit precision
    kSample64: int  # 64-bit precision


# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  AUTOGENERATED CODE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# <litgen_stub> // Autogenerated code below! Do not edit!
####################    <generated_from:VstParameter.h>    ####################

class VstParameter:
    title: str
    short_title: str
    default_normalized_value: float
    value: float
    id: int
    flags: int
    step_count: int
    unit_id: int
    units: str

    # Helper functions to interpret flags
    def can_automate(self) -> bool:
        pass
    def is_bypass(self) -> bool:
        pass
    def is_hidden(self) -> bool:
        pass
    def is_list(self) -> bool:
        pass
    def is_program_change(self) -> bool:
        pass
    def is_read_only(self) -> bool:
        pass
    def is_wrap_around(self) -> bool:
        pass
    def __init__(
        self,
        title: str = "",
        short_title: str = "",
        default_normalized_value: float = float(),
        value: float = float(),
        flags: int = int(),
        step_count: int = int(),
        unit_id: int = int(),
        units: str = ""
        ) -> None:
        """Auto-generated default constructor with named params"""
        pass



####################    </generated_from:VstParameter.h>    ####################


####################    <generated_from:Utils.h>    ####################

####################    </generated_from:Utils.h>    ####################


####################    <generated_from:PluginHost.hpp>    ####################





class VstHost:
    def __init__(
        self,
        samplerate: float = 44100.0,
        max_samples_per_block: int = 512,
        process_mode: int = ProcessModes.kOffline,
        symbolic_sample_size: int = SymbolicSampleSizes.kSample32
        ) -> None:
        pass

    def init(self, path: str) -> bool:
        pass
    def terminate(self) -> None:
        pass



    # param & states
    def get_parameters(self) -> List[VstParameter]:
        pass
    @overload
    def set_parameter(self, id: int, value: float) -> None:
        pass
    @overload
    def set_parameter(self, title: str, value: float) -> None:
        pass
    def save_preset(self, path: str) -> None:
        pass
    def load_preset(self, path: str) -> None:
        pass
    def clear_internal_buffer(self) -> None:
        pass




####################    </generated_from:PluginHost.hpp>    ####################

# </litgen_stub> // Autogenerated code end!